...
function punctualPct(): string {
  if (!props.stats.total_today) return '—';
  return Math.round((props.stats.punctual_today / props.stats.total_today) * 100) + '%';
}
</script>
      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <Link
            ...
            class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-primary-foreground bg-primary hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-all"
          >
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div class="bg-card rounded-xl border border-border shadow-sm p-6 overflow-hidden relative">
            <dt class="text-sm font-medium text-muted-foreground truncate">Total Employees Tracked</dt>
            <dd class="mt-2 text-3xl font-semibold text-card-foreground">{{ stats.total_employees }}</dd>
            <div class="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-indigo-400 to-indigo-600"></div>
          </div>
          <div class="bg-card rounded-xl border border-border shadow-sm p-6 overflow-hidden relative">
            <dt class="text-sm font-medium text-muted-foreground truncate">Punctual Today</dt>
            <dd class="mt-2 text-3xl font-semibold text-emerald-600 dark:text-emerald-400">{{ punctualPct() }}</dd>
            <div class="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-emerald-400 to-emerald-600"></div>
          </div>
          <div class="bg-card rounded-xl border border-border shadow-sm p-6 overflow-hidden relative">
            <dt class="text-sm font-medium text-muted-foreground truncate">Flagged Late Today</dt>
            <dd class="mt-2 text-3xl font-semibold text-amber-600 dark:text-amber-400">{{ stats.flagged_late }}</dd>
            <div class="absolute bottom-0 inset-x-0 h-1 bg-gradient-to-r from-amber-400 to-amber-600"></div>
          </div>
          <div class="bg-card rounded-xl border border-border shadow-sm p-6 overflow-hidden relative">
            <dt class="text-sm font-medium text-muted-foreground truncate">Absences Today</dt>
            <dd class="mt-2 text-3xl font-semibold text-rose-600 dark:text-rose-400">{{ stats.absences_today }}</dd>
        ...
        <div class="bg-card rounded-xl border border-border shadow-sm overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-border text-left text-sm whitespace-nowrap">
              <thead class="bg-muted/50">
                <tr class="text-muted-foreground">
                  ...
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Employee ID</th>
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Time In</th>
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Time Out</th>
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Total Hr</th>
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Tardiness</th>
                  <th class="px-6 py-4 font-semibold uppercase tracking-wider text-xs">Status</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-border bg-card">
                <tr
                  v-for="item in summaries.data"
                  :key="item.userid + '-' + item.record_date"
                  class="hover:bg-muted/50 transition-colors duration-150"
                >
                  <td class="px-6 py-4 font-medium text-foreground">{{ formatDate(item.record_date) }}</td>
                  ...
                  <td class="px-6 py-4">
                    <div v-if="item.is_late" class="flex items-center gap-1.5 text-rose-600 dark:text-rose-400">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-4 h-4">
                        <path fill-rule="evenodd" d="M10 18a8 8 0 1 0 0-16 8 8 0 0 0 0 16zm.75-11.25a.75.75 0 0 0-1.5 0v4.59L7.3 9.24a.75.75 0 0 0-1.1 1.02l3 3.25a.75.75 0 0 0 1.1 0l3-3.25a.75.75 0 1 0-1.1-1.02l-1.95 2.1V6.75z" clip-rule="evenodd" />
                      </svg>
          ...
          <div class="px-6 py-4 border-t border-border bg-muted/20 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span class="text-sm text-muted-foreground">
              Showing <span class="font-medium text-foreground">{{ summaries.from || 0 }}</span>
              to <span class="font-medium text-foreground">{{ summaries.to || 0 }}</span>
              of <span class="font-medium text-foreground">{{ summaries.total }}</span> entries
            </span>
            <div class="flex gap-2">
              <template v-for="(link, index) in summaries.links" :key="index">
                ...
                <span
                  v-else
                  v-html="link.label"
                  class="px-3 py-1 text-sm bg-secondary/50 text-muted-foreground rounded-md cursor-not-allowed"
                />
              </template>
            </div>
          </div>
